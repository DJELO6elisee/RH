...
const AgentFonctionsPage = () => {
    ...
    // Filtres
    const [filterNbFonctions, setFilterNbFonctions] = useState('');
    const [filterNomFonction, setFilterNomFonction] = useState('');
    ...
    const loadAllAgents = useCallback(async () => {
        try {
            ...
            while (hasMore) {
                const response = await fetch(`https://tourisme.2ise-groupe.com/api/fonction-agents/agents?page=${page}&limit=1000${searchParam}`, {
                    headers: {
    ...
    const filteredAgents = useMemo(() => {
        ...
        // Filtre par nombre de fonctions
        if (filterNbFonctions !== '') {
            const nbFonctions = parseInt(filterNbFonctions);
            if (!isNaN(nbFonctions)) {
                filtered = filtered.filter(agent => {
                    const agentNbFonctions = parseInt(agent.nb_fonctions) || 0;
                    if (nbFonctions === 5) {
                        // "5 fonctions ou plus"
                        return agentNbFonctions >= 5;
                    } else {
                        return agentNbFonctions === nbFonctions;
                    }
                });
            }
        }
        
        // Filtre par nom de fonction
        if (filterNomFonction !== '') {
            filtered = filtered.filter(agent => {
                const fonctionNom = (agent.derniere_fonction_nom || '').toLowerCase();
                return fonctionNom.includes(filterNomFonction.toLowerCase());
            });
        }
        
        return filtered;
    }, [allAgents, filterNbFonctions, filterNomFonction]);
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [filterNbFonctions, filterNomFonction]);
    ...
    const fetchFonctions = useCallback(async () => {
        try {
            ...
            let url = 'https://tourisme.2ise-groupe.com/api/fonctions?limit=10000';
    ...
    // Charger les fonctions au montage et quand l'utilisateur (ministère) change
    useEffect(() => {
        fetchFonctions();
    }, [fetchFonctions]);
                    ...
                    <Card>
                        ...
                        <CardBody>
                            ...
                            <Row className="mb-3">
                                <Col md={4}>
                                    <FormGroup>
                                        <Label for="filterNbFonctions">Filtrer par nombre de fonctions</Label>
                                        <Input
                                            type="select"
                                            id="filterNbFonctions"
                                            value={filterNbFonctions}
                                            onChange={(e) => setFilterNbFonctions(e.target.value)}
                                        >
                                            <option value="">Tous</option>
                                            <option value="0">0 fonction</option>
                                            <option value="1">1 fonction</option>
                                            <option value="2">2 fonctions</option>
                                            <option value="3">3 fonctions</option>
                                            <option value="4">4 fonctions</option>
                                            <option value="5">5 fonctions ou plus</option>
                                        </Input>
                                    </FormGroup>
                                </Col>
                                <Col md={6}>
                                    <FormGroup>
                                        <Label for="filterNomFonction">Filtrer par nom de fonction</Label>
                                        <Input
                                            type="text"
                                            id="filterNomFonction"
                                            placeholder="Rechercher une fonction..."
                                            value={filterNomFonction}
                                            onChange={(e) => setFilterNomFonction(e.target.value)}
                                        />
                                    </FormGroup>
                                </Col>
                                <Col md={2} className="d-flex align-items-end">
                                    <Button
                                        color="secondary"
                                        onClick={() => {
                                            setFilterNbFonctions('');
                                            setFilterNomFonction('');
                                        }}
                                        style={{ width: '100%' }}
                                    >
                                        Réinitialiser
                            ...
                            {(filterNbFonctions !== '' || filterNomFonction !== '') && (
                                <Alert color="info" className="mb-3">
                                    ...
                                    {(filterNbFonctions !== '' || filterNomFonction !== '') && (
                                        <span className="ms-2">
                                            {filterNbFonctions !== '' && `• Nombre de fonctions: ${filterNbFonctions === '5' ? '5 ou plus' : filterNbFonctions}`}
                                            {filterNomFonction !== '' && ` • Fonction: "${filterNomFonction}"`}
                                        </span>
                                    )}
                                </Alert>
                            )}
