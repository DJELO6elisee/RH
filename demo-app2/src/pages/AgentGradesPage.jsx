...
const AgentGradesPage = () => {
    ...
    // Filtres
    const [filterNbGrades, setFilterNbGrades] = useState('');
    const [filterNomGrade, setFilterNomGrade] = useState('');
    ...
    const loadAllAgents = useCallback(async () => {
        try {
            ...
            while (hasMore) {
                const response = await fetch(`https://tourisme.2ise-groupe.com/api/grades-agents/agents?page=${page}&limit=1000${searchParam}`, {
                    headers: {
    ...
    const filteredAgents = useMemo(() => {
        ...
        // Filtre par nombre de grades
        if (filterNbGrades !== '') {
            const nbGrades = parseInt(filterNbGrades);
            if (!isNaN(nbGrades)) {
                filtered = filtered.filter(agent => {
                    const agentNbGrades = parseInt(agent.nb_grades) || 0;
                    if (nbGrades === 5) {
                        // "5 grades ou plus"
                        return agentNbGrades >= 5;
                    } else {
                        return agentNbGrades === nbGrades;
                    }
                });
            }
        }
        
        // Filtre par nom de grade
        if (filterNomGrade !== '') {
            filtered = filtered.filter(agent => {
                const gradeNom = (agent.dernier_grade_nom || '').toLowerCase();
                return gradeNom.includes(filterNomGrade.toLowerCase());
            });
        }
        
        return filtered;
    }, [allAgents, filterNbGrades, filterNomGrade]);
                    ...
                    <Card>
                        ...
                        <CardBody>
                            ...
                            <Row className="mb-3">
                                <Col md={4}>
                                    <FormGroup>
                                        <Label for="filterNbGrades">Filtrer par nombre de grades</Label>
                                        <Input
                                            type="select"
                                            id="filterNbGrades"
                                            value={filterNbGrades}
                                            onChange={(e) => setFilterNbGrades(e.target.value)}
                                        >
                                            ...
                                            <option value="5">5 grades ou plus</option>
                                        </Input>
                                    </FormGroup>
                                </Col>
                                <Col md={6}>
                                    <FormGroup>
                                        <Label for="filterNomGrade">Filtrer par nom de grade</Label>
                                        <Input
                                            type="text"
                                            id="filterNomGrade"
                                            placeholder="Rechercher un grade..."
                                            value={filterNomGrade}
                                            onChange={(e) => setFilterNomGrade(e.target.value)}
                                        />
                                    </FormGroup>
                                </Col>
                            ...
                            {(filterNbGrades !== '' || filterNomGrade !== '') && (
                                <Alert color="info" className="mb-3">
                                    ...
                                    {(filterNbGrades !== '' || filterNomGrade !== '') && (
                                        <span className="ms-2">
                                            {filterNbGrades !== '' && `• Nombre de grades: ${filterNbGrades === '5' ? '5 ou plus' : filterNbGrades}`}
                                            {filterNomGrade !== '' && ` • Grade: "${filterNomGrade}"`}
                                        </span>
                                    )}
                                </Alert>
                            )}
