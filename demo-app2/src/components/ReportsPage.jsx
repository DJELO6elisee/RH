...
const ReportsPage = ({ 
    ...
    user,
    /** Si défini (≥1), l’API hierarchical-report exclut les agents dont la retraite tombe dans les N prochaines années */
    hierarchicalExcludeRetirementWithinYears = null,
    /** Bandeau optionnel au-dessus de l’en-tête (ex. contrôle d’horizon projection) */
    topBanner = null
}) => {
    ...
    const loadHierarchicalData = async () => {
        try {
            ...
            // Ne pas réappliquer hasReachedRetirement : diverge du SQL backend et excluait des agents présents dans hierarchyTree.
            setData(dataToSet);
    ...
    const groupAgentsHierarchically = async (data) => {
        ...
        if (hierarchyTree && Array.isArray(hierarchyTree) && hierarchyTree.length > 0) {
            ...
            // Indexer les agents complets par ID (clés nombre / chaîne) pour aligner l'arbre API et filteredData
            const agentMap = {};
            data.forEach((agent) => {
                if (!agent) return;
