...
const SearchableSelect = ({
    ...
    useEffect(() => {
        if (value !== null && value !== undefined && value !== '' && options && options.length > 0) {
            ...
            // Valeur vide ('' ou null) : si une option "Aucune" existe (id === ''), l'afficher
            if ((value === '' || value === null || value === undefined) && options && options.length > 0) {
                const emptyOption = options.find(opt => {
                    const optId = typeof opt === 'object' ? opt.id : opt;
                    return optId === '' || optId === null || optId === undefined;
                });
                if (emptyOption) {
