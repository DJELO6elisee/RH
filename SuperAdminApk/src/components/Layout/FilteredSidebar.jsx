...
const navItems = [
    { to: '/dashboard', name: 'Tableau de Bord', exact: true, Icon: MdDashboard },
];
...
const getFilteredRoutesBase = (user, assignedRouteIds = []) => {
    ...
    const allowedRoutes = {
        'ministere': [
            ...
            'historiques-demandes', 'decision', 'certificat-prise-service'
        ],
        'institution': [
    ...
    if (isDRH || isDirecteur) {
        return backendRoutes.filter(route => {
            // Vérifier d'abord l'accès basé sur le rôle
            if (!hasRoleAccess(route)) {
                return false;
            }
...
const FilteredSidebar = (props) => {
    ...
    if (isDRH || isDirecteur) {
        ...
        finalDemandesRoutes = backendRoutes.filter(route => {
            if (route.category !== 'Gestions des documents administratifs') {
                return false;
            }
            // Vérifier si l'utilisateur a accès selon le rôle de la route
            if (!route.roles || route.roles.length === 0) {
                return true; // Pas de restriction de rôle
            }
            const userRoleLower = user.role?.toLowerCase();
