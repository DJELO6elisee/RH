...
const CreateDemandeModal = ({ isOpen, toggle, onDemandeCreated, agentId }) => {
    ...
    /** Jours restants par année (ex: { 2023: 30, 2024: 15, 2025: 30 }) pour affichage "Disponible pour l'année X" */
    const [congesParAnnee, setCongesParAnnee] = useState({});
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (result.success && result.data) {
                    ...
                    setCongesParAnnee({
                        [yearBeforePrevious]: congesAnnee2.jours_restants ?? 30,
                        [previousYear]: congesAnnee1.jours_restants ?? 30,
                        [currentYear]: congesCourante.jours_restants ?? 30
                    });
                } else {
                    setJoursRestants(30);
                    setJoursRestantsAnneesPrecedentes({
                            ...
                            {isCertificatCessation && (
                                <>
                                    ...
                                    <Row>
                                        ...
                                        {['congé annuel', 'congé partiel', 'congé exceptionnel'].includes(formData.motif_conge) && (
                                            <Col md="6">
                                                <FormGroup>
                                                    <Label for="annee_au_titre_conge">Année au titre de laquelle le congé est demandé *</Label>
                                                    <Input
                                                        type="select"
                                                        name="annee_au_titre_conge"
                                                        id="annee_au_titre_conge"
                                                        value={formData.annee_au_titre_conge}
                                                        onChange={handleInputChange}
                                                        required
                                                    >
                                                        <option value="">Sélectionner l'année</option>
                                                        {(formData.motif_conge === 'congé exceptionnel'
                                                    ...
                                                    <small className="form-text text-muted">
                                                        {formData.motif_conge === 'congé exceptionnel'
                                                            ? 'Pour le congé exceptionnel, l\'année en cours peut être retenue. Détermine l\'année de déduction et le numéro de décision.'
                                                            : 'Années précédentes uniquement (l\'année en cours ne s\'applique qu\'au congé exceptionnel). Détermine l\'année de déduction et le numéro de décision.'
                                                        }
                                                    </small>
                                                </FormGroup>
                                            </Col>
