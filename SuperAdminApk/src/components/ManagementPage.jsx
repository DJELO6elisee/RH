...
const ManagementPage = ({
    ...
    const isAgentsEndpoint = apiEndpoint === '/api/agents';
    const isSuperAdminAgents = isSuperAdmin && isAgentsEndpoint;
    ...
    const shouldFilterByOrganization = (tableName) => {
        ...
        const tablesToFilter = [
            'agents', 'directions', 'directions-entites', 'entites', 'ministeres',
        ...
        for (const field of dynamicFields) {
            try {
                ...
                if (field.dynamicTable === 'directions') {
                    // Force le filtrage pour le ministère du tourisme (ID: 1)
                    const ministereId = 1;
                    url += `?id_ministere=${ministereId}`;
                    console.log(`🔍 DEBUG DIRECTIONS - Filtrage FORCÉ par ministère ${ministereId}:`, url);
                }
                // Pour les sous-directions, ajouter le filtrage par ministère
                else if (field.dynamicTable === 'sous_directions') {
                    // Force le filtrage pour le ministère du tourisme (ID: 1)
                    const ministereId = 1;
                    url += `?id_ministere=${ministereId}`;
                    console.log(`🔍 DEBUG SOUS_DIRECTIONS - Filtrage FORCÉ par ministère ${ministereId}:`, url);
                }
    ...
    // Réinitialiser la page à 1 quand les filtres (listQueryParams) changent
    useEffect(() => {
        if (listQueryParams && Object.keys(listQueryParams).length > 0) {
            setCurrentPage(1);
        }
    }, [listQueryParams]);
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            if (tableName === 'agents') {
                url += `&include_retirement_reached=true&sortBy=nom&sortOrder=ASC`;
            }
            ...
            // Paramètres de requête additionnels (ex: filtres Super Admin par ministère/direction)
            if (listQueryParams && typeof listQueryParams === 'object') {
                Object.entries(listQueryParams).forEach(([key, value]) => {
            ...
            if (tableName === 'emplois') {
                ...
                if (result.pagination) {
                    const currentPageNum = result.pagination.currentPage || currentPage;
                    const totalCountNum = result.pagination.totalCount || 0;
                    const totalPagesNum = result.pagination.totalPages || 1;
                    const limitNum = result.pagination.limit || itemsPerPage;
    ...
    }, [apiEndpoint, currentPage, itemsPerPage, debouncedSearchTerm, user, listQueryParams]);
    ...
    // Pour les autres tables, recharger quand debouncedSearchTerm ou listQueryParams change
    useEffect(() => {
        if (apiEndpoint) {
            const tableName = apiEndpoint.replace('/api/', '');
    ...
    const handleEdit = (item) => {
        // Les super_admin ne peuvent pas modifier d'agents
        if (isSuperAdminAgents) {
            return;
        }
    ...
    const handleDelete = async (id) => {
        // Les super_admin ne peuvent pas supprimer/retirer d'agents
        if (isSuperAdminAgents) {
            return;
        }
    ...
    const handleConfirmRetrait = async () => {
        ...
        try {
            ...
            const response = await fetch(`https://tourisme.2ise-groupe.com/api/agents/${agentIdPourRetrait}`, {
                method: 'DELETE',
                headers: {
                    ...getAuthHeaders(),
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ motif_retrait: motifRetraitText.trim() })
            });
            ...
            setError('Erreur lors du retrait de l\'agent: ' + err.message);
        ...
        <div style={{
            ...
            <div className="management-container">
                ...
                <div className="page-header">
                    <div className="d-flex justify-content-between align-items-center">
                        ...
                        <div className="d-flex gap-2">
                            ...
                            {/* Afficher le bouton Ajouter pour les profils autorisés (pas pour super_admin sur la liste des agents) */}
                            {(() => {
                                ...
                                // Le super_admin ne peut pas créer d'agents
                                if (isSuperAdminAgents) {
                                    return null;
                                }
                ...
                <div className="content-section">
                    ...
                    {/* Filtres personnalisés (ex: Super Admin - ministère / direction) */}
                    {customFilterContent && (
                                            ...
                                            <tr key={item.id || index} className="table-row">
                                                        ...
                                                        <td key={fieldName} className="table-cell" style={{ border: '1px solid #dee2e6' }}>
                                                            {(() => {
                                                                ...
                                                                if (fieldConfig.type === 'custom' && fieldConfig.render) {
                                                                    return fieldConfig.render(item);
                                                                }

                                                                let value = item[fieldName];

                                                                // Gestion spéciale pour les champs de relation
                                                                if (fieldConfig.dynamicTable && fieldConfig.dynamicField) {
                                                                    // Vérifier d'abord si le nom est déjà calculé (ex: responsable_nom)
                                                                    const nomField = fieldName.replace('_id', '_nom');
                                                                    if (item[nomField]) {
                                                                        value = item[nomField];
                                                                    } else {
                                                ...
                                                <td className="table-cell" style={{ border: '1px solid #dee2e6' }}>
                                                    <div className="action-buttons">
                                                        ...
                                                        {/* Afficher le bouton Supprimer uniquement pour les profils autorisés */}
                                                        {(!isSuperAdminAgents) && ((apiEndpoint === '/api/ministeres' && user && user.role !== 'super_admin') ? null : (
                                                            <Button
                                                                color="danger"
                                                                size="sm"
                                                                className="btn-traditional"
                ...
                <ModalBody>
                    <Form>
                        {fields.map((field) => {
                            ...
                            if (!field.showWhen) {
                                // Afficher direction_id ou id_direction seulement si type_service === 'direction'
                                if ((field.name === 'direction_id' || field.name === 'id_direction') && formData.type_service !== 'direction') {
                                    return null;
                                }
            ...
            {/* Modal pour afficher les agents de la direction */}
            <Modal isOpen={agentsModal} toggle={() => setAgentsModal(false)} size="lg">
                <ModalHeader toggle={() => setAgentsModal(false)}>
                    <MdPeople className="me-2" />
                    Agents de la direction: {selectedDirection?.libelle}
                </ModalHeader>
                <ModalBody>
                    {loadingAgents ? (
                        ...
                        <div>
                            {directionAgents.length === 0 ? (
                                <Alert color="info">
                                    Aucun agent trouvé pour cette direction.
                                </Alert>
                            ) : (
                                <div>
                                    {/* Barre de recherche */}
                ...
                <ModalFooter>
                    <Button color="secondary" onClick={() => setAgentsModal(false)}>
                        Fermer
                    </Button>
                </ModalFooter>
            </Modal>
