...
const AgentsPage = () => {
    ...
    // Filtres Super Admin : ministère, direction, sous-direction
    ...
    // Charger la liste des ministères (Super Admin uniquement)
    useEffect(() => {
    ...
    // Charger les directions du ministère sélectionné
    useEffect(() => {
    ...
    // Paramètres de requête pour l'API agents (stables pour éviter re-renders inutiles)
    ...
    // UI des filtres Super Admin (ministère, direction, sous-direction)
    ...
    const handleAgentSubmit = async (formData) => {
        try {
            ...
            const numericFields = ['id_direction', 'id_sous_direction', 'sous_direction_id', 'service_id', 'id_service',
                ...
                'id_position', 'id_handicap', 'id_mode_entree', 'echelon_prefectoral'];
            ...
            if (result.id || result.success) {
                ...
                const RELOAD_DELAY = 30000; // 30 secondes pour permettre de voir les logs
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                const formattedDiplomes = result.data.diplomes?.map((diplome, index) => ({
                    ...
                    date_diplome: formatDateForInput(diplome.date_diplome) || '',
                    ecole: diplome.ecole || '',
                    ville: diplome.ville || '',
                    pays: diplome.pays || '',
                ...
                const transformedData = {
                    ...
                    // Mapper explicitement la direction pour s'assurer qu'elle est présente
    ...
    return (
        <>
            <ManagementPage
                title="Agents"
                description={isSuperAdmin ? "Gestion des agents de tous les ministères — filtrez par ministère, direction et sous-direction" : "Gestion complète des agents et employés du Ministère du Tourisme et des Loisirs"}
                icon={MdPerson}
