...
const AttributionTachesPage = () => {
    ...
    const availableRoutes = backendRoutes.filter(route => {
        // Exclure uniquement les routes système explicites
        if (route.id === 'agent-dashboard' || route.id === 'drh-dashboard') {
            return false;
        }
        // Inclure toutes les routes qui ont un id valide (même sans category)
        if (!route.id) {
            return false;
        }
        ...
        // Inclure toutes les autres routes (même sans category)
        return true;
    });
