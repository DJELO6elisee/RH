...
const AgentsReportsPage = ({ retirementProjection = false }) => {
    ...
    // Charger la liste des ministères (pour le super admin)
    useEffect(() => {
        const fetchMinisteres = async () => {
            if (user?.role === 'super_admin') {
                try {
                    ...
                    const result = await response.json();
                    if (result.success && Array.isArray(result.data)) {
                        setMinisteres(result.data);
                    } else if (Array.isArray(result)) {
                        setMinisteres(result);
                    }
                } catch (error) {
        ...
        const fetchDirectionsGenerales = async () => {
            try {
                ...
                if (user?.role === 'super_admin' && selectedMinistere) {
                    apiUrl += `&id_ministere=${selectedMinistere}`;
                } else if (user?.organization?.type === 'ministere' && user?.organization?.id) {
    ...
    useEffect(() => {
        ...
        fetchSousDirections();
    }, [user, selectedDirection, selectedMinistere]);
    ...
    useEffect(() => {
        ...
        fetchServices();
    }, [selectedDirection, user, selectedMinistere]);
    ...
    const filters = [
        ...(user?.role === 'super_admin' ? [{
            ...
            options: [
                ...
                ...ministeres.map(m => ({
                    value: m.id,
                    label: m.nom || m.libelle || m.code || m.id
                }))
            ],
            onChange: (value) => {
                setSelectedMinistere(value);
            }
        }] : []),
        {
            name: 'type_agent',
