...
const RetraitesPage = () => {
    ...
    const [filterValues] = useState({ 
        ...
        // Appliquer automatiquement le filtre ministère de l'utilisateur connecté
        ...(user?.organization?.type === 'ministere' && { id_ministere: user.organization.id })
    });
    ...
    const loadStats = async () => {
        try {
            const response = await fetch('https://tourisme.2ise-groupe.com/api/agents/retirement-stats', {
                headers: getAuthHeaders()
            });
            const result = await response.json();
    ...
    const handleBatchCalculate = async () => {
        ...
        try {
            setCalculating(true);
            const response = await fetch('https://tourisme.2ise-groupe.com/api/agents/batch-calculate-retirement', {
                method: 'POST',
                headers: getAuthHeaders()
            });
    ...
    const loadProjection = async (years) => {
        try {
            const response = await fetch(`https://tourisme.2ise-groupe.com/api/agents/retirement-projection?years=${years}`, {
                headers: getAuthHeaders()
            });
            const result = await response.json();
