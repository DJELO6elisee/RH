...
const VerificationRetraitesPage = () => {
    ...
    const handleExportJobProjectionExcel = () => {
        ...
        worksheet['!cols'] = [
            { wch: 50 }, // Augmenté pour permettre l'affichage complet des noms d'emploi
            { wch: 18 },
            { wch: 22 },
            { wch: 22 },
            { wch: 32 },
        ...
        const merges = [];
        const numCols = 8; // Nombre de colonnes (A à H = 0 à 7)
    ...
    const groupedData = useMemo(() => {
        ...
        displayedAgents.forEach((agent) => {
            const directionName =
                agent.direction_libelle?.trim() || DEFAULT_LABELS.direction;
            const serviceName =
                agent.service_libele?.trim() ||
                agent.service_libelle?.trim() ||
                DEFAULT_LABELS.service;
    ...
    const handleExportProjectionSummaryExcel = () => {
        ...
        const rows = [];
        const generationDate = new Date();
        rows.push(['ÉTAT DES AGENTS - RAPPORT HIÉRARCHIQUE']);
        rows.push([
            `Généré le ${generationDate.toLocaleDateString('fr-FR')} à ${generationDate.toLocaleTimeString(
                'fr-FR'
            )}`
        ...
        const pushAgentRow = (agent) => {
            const matricule = agent.matricule || '-';
            const civilite =
                agent.civilite_libelle ||
                agent.civilite_libele ||
                agent.civilite ||
                '-';
            const sexe = formatAgentSex(agent);
            const nom = agent.nom || '-';
            const prenom = agent.prenom || '-';
            const fonction = formatAgentFunction(agent);
        ...
        const addAgentsTableHeader = () => {
            rows.push(['N°', 'Matricule', 'Civilité', 'Sexe', 'Nom', 'Prénoms', 'Fonction']);
        };

        const isDefaultService = (label) =>
            normalizeLabel(label || '') === normalizeLabel(DEFAULT_LABELS.service);
        const isDefaultSousDirection = (label) =>
            normalizeLabel(label || '') === normalizeLabel(DEFAULT_LABELS.sousDirection);

        Object.entries(groupedDirections || {}).forEach(([directionName, directionData]) => {
            rows.push([`DIRECTION : ${directionName}`]);
            rows.push([]);

            const directAgents = directionData.agents.filter(
                (agent) =>
                    isDefaultService(getServiceLabelFromAgent(agent)) &&
                    isDefaultSousDirection(getSousDirectionLabelFromAgent(agent))
            );
            if (directAgents.length > 0) {
                addAgentsTableHeader();
                directAgents.forEach(pushAgentRow);
                rows.push([`Total agents directs direction : ${directAgents.length} agent(s)`]);
                rows.push([]);
            }

            Object.entries(directionData.services || {}).forEach(([serviceName, serviceData]) => {
                const isServicePlaceholder = isDefaultService(serviceName);
                const directServiceAgents = serviceData.agents.filter((agent) =>
                    isDefaultSousDirection(getSousDirectionLabelFromAgent(agent))
                );
                const hasNonDefaultSousDirections = Object.keys(serviceData.sousDirections || {}).some(
                    (sousDirectionName) => !isDefaultSousDirection(sousDirectionName)
                );

                if (isServicePlaceholder) {
                    if (hasNonDefaultSousDirections) {
                        Object.entries(serviceData.sousDirections || {})
                            .filter(([sousDirectionName]) => !isDefaultSousDirection(sousDirectionName))
                            .forEach(([sousDirectionName, sousDirectionData]) => {
                                rows.push([`SOUS-DIRECTION : ${sousDirectionName}`]);
                                rows.push([]);
                                addAgentsTableHeader();
                                sousDirectionData.agents.forEach(pushAgentRow);
                                rows.push([
                                    `Total sous-direction ${sousDirectionName} : ${sousDirectionData.total} agent(s)`
                                ]);
                                rows.push([]);
                            });
                    }
                ...
                const shouldDisplayServiceSection =
                    serviceData.total > 0 || hasNonDefaultSousDirections;

                if (shouldDisplayServiceSection) {
                    rows.push([
                        `SERVICE : ${serviceName}`
                    ]);
                    rows.push([]);
                ...
                if (directServiceAgents.length > 0) {
                    addAgentsTableHeader();
                    directServiceAgents.forEach(pushAgentRow);
                    rows.push([
                        `Total agents directs service : ${directServiceAgents.length} agent(s)`
                    ]);
                    rows.push([]);
                }
                ...
                if (shouldDisplayServiceSection) {
                    rows.push([
                        `TOTAL SERVICE ${serviceName} : ${serviceData.total} agent(s)`
                    ]);
                    rows.push([]);
                }
            });

            rows.push([`TOTAL DIRECTION : ${directionData.total} agent(s)`]);
            rows.push([]);
        });

        rows.push(['', '', '', '', '', '', '']);
        rows.push([`TOTAL GÉNÉRAL : ${displayedAgents.length} agent(s)`]);
        ...
        const today = new Date().toISOString().split('T')[0];
        XLSX.writeFile(workbook, `Rapport_Hierarchique_Projection_${today}.xlsx`);
    };
                    ...
                    <CardBody>
                        <div className="table-responsive">
                            <Table bordered hover size="sm">
                                ...
                                <tbody>
                                    {projectionHierarchyRows.map((row, index) => {
                                        ...
                                        if (row.type === 'agent') {
                                            ...
                                            return (
                                                <tr key={key}>
                                                    ...
                                                    <td>{row.direction}</td>
                                                    <td>{row.service}</td>
                                                    <td>{row.sousDirection}</td>
                                                    <td>{agent.matricule || '-'}</td>
                                                    <td>{`${agent.nom || ''} ${agent.prenom || ''}`.trim() || '-'}</td>
                                                    <td>{formatRetirementDate(agent)}</td>
                                                    <td />
                                                </tr>
                                            );
                                        }
                                        ...
                                        return (
                                            <tr key={key} className="table-light">
                                                <td>{levelLabels[row.type]}</td>
                                                <td>{row.direction}</td>
                                                <td>{row.type !== 'direction' ? row.service : ''}</td>
                                                <td>{row.type === 'sousDirection' ? row.sousDirection : ''}</td>
                                                <td colSpan={2} />
                                                <td />
            ...
            {hasJobProjection && (
                <Card className="mb-4">
                    <CardHeader>
                        <div className="d-flex justify-content-between align-items-center">
                            <CardTitle className="mb-0">
                                Agents partant à la retraite par emploi (horizon {currentFilters.periodYears} an
                                {Number(currentFilters.periodYears) > 1 ? 's' : ''})
                            </CardTitle>
                            <ButtonDropdown
                                isOpen={jobExportDropdownOpen}
                                toggle={() => setJobExportDropdownOpen((prev) => !prev)}
                            >
                                <Button color="success" onClick={handlePrintJobProjection}>
                                    <MdPrint className="me-1" />
                                    Imprimer
                                </Button>
                                <DropdownToggle split color="success" caret />
                                <DropdownMenu end>
                                    <DropdownItem onClick={handleExportJobProjectionExcel}>
                                        Excel (.xlsx)
                                    </DropdownItem>
                                    <DropdownItem onClick={handleExportJobProjectionWord}>
                                        Word (.doc)
                                    </DropdownItem>
                                </DropdownMenu>
                            </ButtonDropdown>
                    ...
                    <CardBody>
                        {projectionJobs.map((job) => {
                            const isExpanded = !!expandedJobs[job.jobName];
                            return (
                                <div key={job.jobName} className="mb-3 border rounded" style={{ borderColor: '#e2e8f0' }}>
                                    <div
                                        className="d-flex justify-content-between align-items-center p-3"
                                        style={{ cursor: 'pointer', backgroundColor: '#f8fafc' }}
                                        onClick={() => toggleJobGroup(job.jobName)}
                                    >
                                        <div>
                                            <strong>{job.jobName}</strong>{' '}
                                            <Badge pill color="primary">
                                                {job.count} agent{job.count > 1 ? 's' : ''}
                                            </Badge>
                                        </div>
                                            ...
                                            <Table bordered hover size="sm">
                                                ...
                                                <tbody>
                                                    {job.agents.map((agent) => (
                                                        <tr key={`${job.jobName}-${agent.id}`}>
                                                            <td>{agent.matricule || '-'}</td>
                                                            <td>{agent.nom || '-'}</td>
                                                            <td>{agent.prenom || '-'}</td>
                                                            <td>{getDirectionLabelFromAgent(agent)}</td>
                                                            <td>{getServiceLabelFromAgent(agent)}</td>
                                                            <td>{getSousDirectionLabelFromAgent(agent)}</td>
                                                            <td>{formatRetirementDate(agent)}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
