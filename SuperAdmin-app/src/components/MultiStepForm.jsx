...
const DiplomesFields = React.memo(({ value = [], onChange, nombreDiplomes, invalid, existingDiplomes = [], errors = {} }) => {
    ...
    // Fonction pour formater les dates pour l'input HTML
            ...
            for (let i = 0; i < nombreDiplomes; i++) {
                if (existingDiplomes[i]) {
                    ...
                    initialDiplomes[i] = {
                        ...
                        date_diplome: formatDateForInput(existingDiplomes[i].date_diplome) || '',
                ...
                <Card key={`diplome-${index}`} className="mb-3">
                    ...
                    <CardBody>
                        <Row>
                            ...
                            <Col md={6}>
                                <FormGroup>
                                    <Label for={`diplome_${index}_date`}>Date d'obtention</Label>
                                    <Input
                                        type="date"
...
const MultiStepForm = ({
    ...
    const handleCascadeChange = async (fieldName, value, currentFormData) => {
        ...
        steps.forEach(step => {
            step.fields.forEach(field => {
                ...
                if (field.dependsOn === fieldName && field.dynamicTable) {
                    ...
                    // Lors de l'édition, préserver la valeur existante
                    const hasExistingValue = currentFormData[field.name] && currentFormData[field.name] !== '';
                    const isEditMode = initialData && Object.keys(initialData).length > 0;
                    
                    if (!isEditMode || !hasExistingValue) {
    ...
    const loadCascadeOptions = async (field, parentValue, optionsObj, currentFormData = {}) => {
        ...
        try {
            ...
            if (field.dependsOn === 'id_direction') {
        ...
        for (const [tableName, fields] of Object.entries(fieldsByTable)) {
            ...
            try {
                ...
                if (['directions', 'services', 'sous-directions'].includes(tableName)) {
                    // Pour ces tables, filtrer dynamiquement par le ministère de l'utilisateur connecté
                    const userMinistereId = await getUserMinistereId();
                    if (userMinistereId) {
                        ministereId = userMinistereId;
                        console.log(`🔍 MultiStepForm - Filtrage par ministère ${ministereId} pour ${tableName}`);
                    } else {
                        console.warn(`⚠️ MultiStepForm - Aucun ministère trouvé pour l'utilisateur, ${tableName} non filtré par ministère`);
                    }
                } else if (tableName === 'agents') {
                    // Pour les agents, récupérer aussi le ministère de l'utilisateur
                    const userMinistereId = await getUserMinistereId();
                    if (userMinistereId) {
                        ministereId = userMinistereId;
                ...
                // Pour les autres tables avec endpoint /select/all
                else if (['categories', 'grades', 'echelons', 'civilites', 'nationalites', 'pays', 'situation_matrimonials', 'type_d_agents', 'mode_d_entrees', 'positions', 'niveau_informatiques', 'pathologies', 'handicaps', 'langues', 'niveau_langues', 'logiciels'].includes(tableName)) {
                    apiUrl = `https://tourisme.2ise-groupe.com/api/${tableName}/select/all`;
                }
    ...
    const loadDynamicOptionsForEdit = async (editData) => {
        ...
        for (const [tableName, fields] of Object.entries(fieldsByTable)) {
            ...
            try {
                ...
                if (tableName === 'directions') {
                    ...
                    // Utiliser uniquement le ministère de l'utilisateur connecté (pas de valeur forcée)
                    const userMinistereId = await getUserMinistereId();
                    if (userMinistereId) {
                        filterParams.push(`id_ministere=${userMinistereId}`);
                        console.log(`🔍 MultiStepForm - Chargement des directions avec endpoint /select/all et filtre par ministère (id_ministere=${userMinistereId})`);
                    } else {
        ...
        if (editData.id_direction) {
            ...
            await handleCascadeChange('id_direction', editData.id_direction, editData);
    ...
    const validateCurrentStep = () => {
        ...
        currentStepFields.forEach(field => {
            ...
            // Vérifier la condition pour conditionalRequired
            if (field.conditionalRequired) {
                const { field: conditionalField, value, values, notValue, notValues } = field.conditionalRequired;
                const conditionalValue = formData[conditionalField];
                
                if (value !== undefined) {
                    isRequired = conditionalValue === value;
                } else if (values !== undefined) {
                    isRequired = values.includes(conditionalValue);
                } else if (notValue !== undefined) {
                    isRequired = conditionalValue !== notValue;
                } else if (notValues !== undefined) {
                    isRequired = !notValues.includes(conditionalValue);
                }
            }
    ...
    const handleSubmit = async () => {
        if (validateCurrentStep()) {
            ...
            try {
                // onSubmit peut maintenant renvoyer une promesse et, en cas d'erreur métier,
                // soit lever une exception, soit renvoyer un objet { success: false, message }
                const result = await onSubmit(formDataToSubmit);

                // Si le parent renvoie explicitement une erreur, l'afficher et NE PAS réinitialiser le formulaire
                ...
                // Succès : réinitialiser l'état du formulaire
                setSubmitError(null);
                setCurrentStep(0);
                setFormData({});
                setErrors({});
                ...
                // En cas d'erreur (HTTP ou autre), ne pas vider le formulaire, mais afficher le message
                console.error('❌ MultiStepForm - Erreur lors de la soumission du formulaire:', error);

                // Essayer de récupérer un message explicite (par ex. contrainte d'unicité du matricule)
                ...
                setSubmitError(message);
                // Ne pas reset le formulaire : l'utilisateur peut cliquer sur "Précédent" et corriger
            }
        }
    };
