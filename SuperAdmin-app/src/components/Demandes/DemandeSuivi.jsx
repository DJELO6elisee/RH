...
const niveauConfig = {
    ...
    valide_par_superieur: { color: 'success', label: 'Validé supérieur' },
    valide_par_sous_directeur: { color: 'success', label: 'Validé sous-directeur' },
    valide_par_directeur: { color: 'success', label: 'Validé directeur' },
    valide_par_drh: { color: 'success', label: 'Validé DRH' },
    valide_par_direction: { color: 'success', label: 'Validé direction' },
    valide_par_dir_cabinet: { color: 'success', label: 'Validé Dir Cabinet' },
    valide_par_chef_cabinet: { color: 'success', label: 'Validé Chef Cabinet' },
    valide_par_directeur_central: { color: 'success', label: 'Validé Dir Central' },
    valide_par_directeur_general: { color: 'success', label: 'Validé Dir Général' },
    valide_par_ministre: { color: 'success', label: 'Validé ministre' },
...
const getProgressValue = (phase, niveau, niveauActuel) => {
    const current = niveauActuel || niveau;
    if (phase === 'retour') {
        return progressionRetour[current] ?? (current === 'finalise' ? 100 : 50);
    }
    return progressionAller[current] ?? (current === 'finalise' ? 100 : 40);
};
...
const renderPeriode = (demande) => {
    if ((demande.type_demande || '').toLowerCase() === 'certificat_reprise_service') {
        return (
            <div>
                <div><strong>Fin congés :</strong> {demande.date_fin_conges ? formatDate(demande.date_fin_conges, false) : 'Non renseignée'}</div>
                        ...
                        <div className="table-responsive">
                            ...
                            >
                                ...
                                <tbody>
                                    ...
                                    const niveauKey = sanitizeKey(demande.niveau_evolution_demande || demande.niveau_actuel || demande.niveau);
                                    const niveauActuelKey = sanitizeKey(demande.niveau_actuel || demande.niveau);
                                    const motif =
