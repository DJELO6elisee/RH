...
const DocumentsGenerated = ({ typeDemande = 'absence', forceAgentView = false }) => {
    const { user } = useAuth();
    ...
    const renderPeriod = (document) => {
        ...
        if (['certificat_reprise_service', 'autorisation_reprise_service'].includes(normalizedType)) {
            const finConges = document.date_fin_conges || document.date_fin;
            const reprise = document.date_reprise_service || document.date_debut;
            const finLabel = finConges ? new Date(finConges).toLocaleDateString('fr-FR') : 'Non renseignée';
            const repriseLabel = reprise ? new Date(reprise).toLocaleDateString('fr-FR') : 'Non renseignée';
            return `Fin de congé : ${finLabel} | Reprise : ${repriseLabel}`;
        }
        ...
        if (debut && fin) {
            const debutDate = new Date(debut).toLocaleDateString('fr-FR');
            const finDate = new Date(fin).toLocaleDateString('fr-FR');
            return `${debutDate} - ${finDate}`;
        }
        if (debut) {
            return new Date(debut).toLocaleDateString('fr-FR');
        }
        if (fin) {
            return new Date(fin).toLocaleDateString('fr-FR');
        }
        return 'N/A';
    };
