...
const DemandesDRHList = ({ onDemandeClick, typeDemande = '' }) => {
    ...
    const loadDemandes = async () => {
        ...
        try {
            ...
            console.log('🔍 Présence du champ debug:', !!data.debug);
            console.log('🔍 Contenu complet de la réponse:', JSON.stringify(data, null, 2));
            
            // Afficher les informations de debug si disponibles
            if (data.debug) {
                console.log('🔍 ========== DEBUG INFO ==========');
                console.log('🔍 DRH ID:', data.debug.drhId);
                console.log('🔍 DRH Direction ID:', data.debug.drhDirectionId);
                console.log('🔍 isDRH:', data.debug.isDRH);
                console.log('🔍 Rôle nom:', data.debug.roleNom);
                console.log('🔍 Rôle nom (lowercase):', data.debug.roleNomLowercase);
                console.log('🔍 Validateur ID Direction:', data.debug.validateurIdDirection);
                console.log('🔍 Nombre de résultats retournés:', data.debug.resultCount);
                console.log('🔍 Total demandes valide_par_sous_directeur:', data.debug.totalValideParSousDirecteur || 0);
                console.log('🔍 Demandes valide_par_sous_directeur dans direction DRH:', data.debug.valideParSousDirecteurInDRHDirection || 0);
                if (data.debug.demandesValideParSousDirecteur) {
                    console.log('🔍 Détails des demandes valide_par_sous_directeur:', data.debug.demandesValideParSousDirecteur);
                }
                if (data.debug.demandesValideParSousDirecteurInDRH) {
                    console.log('🔍 Demandes valide_par_sous_directeur dans direction DRH:', data.debug.demandesValideParSousDirecteurInDRH);
                }
                if (data.debug.demandesSoumisParSousDirDRH) {
                    console.log('🔍 Nombre de demandes soumis créées par sous-directeurs de la direction DRH:', data.debug.demandesSoumisParSousDirDRH.length);
                    console.log('🔍 Détails des demandes soumis créées par sous-directeurs:', data.debug.demandesSoumisParSousDirDRH);
                }
                console.log('🔍 ================================');
            } else {
                console.log('⚠️ Aucune information de debug disponible dans la réponse');
            }
    ...
    const getNiveauEvolutionBadge = (niveau, phase) => {
        const niveaux = {
            ...
            'valide_par_superieur': { color: 'warning', text: 'En attente supérieur', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
            'valide_par_sous_directeur': { color: 'warning', text: 'En attente sous-directeur', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
            'valide_par_directeur': { color: 'warning', text: 'En attente directeur', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
            'valide_par_drh': { color: 'warning', text: 'En attente DRH', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
            'valide_par_dir_cabinet': { color: 'warning', text: 'En attente Dir Cabinet', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
            'valide_par_chef_cabinet': { color: 'warning', text: 'En attente Chef Cabinet', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
            'valide_par_directeur_general': { color: 'warning', text: 'En attente Dir Général', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
            'valide_par_directeur_central': { color: 'warning', text: 'En attente directeur central', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
            // Pour le DRH, une demande avec niveau_evolution_demande = 'valide_par_directeur_service_exterieur'
            // signifie que le Directeur des services extérieurs a déjà validé et que la demande
            // est maintenant en attente de validation DRH.
            'valide_par_directeur_service_exterieur': { color: 'warning', text: 'En attente DRH', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
            'valide_par_ministre': { color: 'warning', text: 'En attente ministre', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
            'retour_dir_cabinet': { color: 'warning', text: 'Retour Dir Cabinet', style: { color: '#000', fontWeight: 'bold', backgroundColor: '#fff3cd' } },
                                                ...
                                                {filters.statut === 'historique' && demande.niveau_libelle ? (
                                                    ...
                                                    <div style={{ 
                                                        ...
                                                        {(() => {
                                                            ...
                                                            const isDirecteurCentral = roleNorm === 'directeur_central';
                                                            ...
                                                            const niveauForBadge = (isDirecteurCentral && isWaitingForDirector) ? 'valide_par_directeur_central' : demande.niveau_evolution_demande;
                                                            return getNiveauEvolutionBadge(niveauForBadge, demande.phase);
                                                        })()}
                                                    </div>
                                                )}
                                            </div>
