...
const LoginPage = () => {
  ...
  // Utiliser l'organisation du domaine en priorité, sinon celle de l'URL
  const organizationType = domainOrganization?.type || urlOrganizationType;
  const organizationId = domainOrganization?.id || urlOrganizationId;
      ...
      if (result.success) {
        ...
        if (userRole === 'super_admin') {
          ...
          const agentDashboardUrl = organizationId && organizationType 
            ...
            : '/agent-dashboard';
          console.log(`🔀 Redirection ${userRoleRaw} (normalisé: ${userRole}) vers:`, agentDashboardUrl);
    ...
    >
      ...
      <Navbar className="navbar-login" expand="md">
        <Container>
          ...
          >
            <img 
              src={organization?.logo_url || process.env.PUBLIC_URL + '/img/logo-tourisme.jpg'} 
              alt={`Logo ${organization?.nom || domainOrganization?.name || 'Ministère'}`} 
              className="navbar-logo me-2"
              style={{ height: '40px', width: 'auto' }}
            />
            {'Ministère du Tourisme et des Loisirs' || organization?.nom || domainOrganization?.name  }
          </NavbarBrand>
          <Link 
            to={getHomeUrl()} 
