-- Script SQL sécurisé pour ajouter les champs de type de service
-- Ce script gère les cas où les colonnes et contraintes existent déjà
...
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_type_service' AND conrelid = 'services'::regclass) THEN
        ALTER TABLE services DROP CONSTRAINT check_type_service;
    END IF;
END $$;

-- Ajouter la contrainte de vérification pour le type de service
ALTER TABLE services ADD CONSTRAINT check_type_service 
    CHECK (type_service IN ('direction', 'sous_direction'));

-- Supprimer les contraintes de clés étrangères si elles existent déjà, puis les recréer
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_services_direction' AND conrelid = 'services'::regclass) THEN
        ALTER TABLE services DROP CONSTRAINT fk_services_direction;
    END IF;
END $$;

DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_services_sous_direction' AND conrelid = 'services'::regclass) THEN
        ALTER TABLE services DROP CONSTRAINT fk_services_sous_direction;
    END IF;
END $$;
...
-- Créer les index (IF NOT EXISTS gère automatiquement les doublons)
