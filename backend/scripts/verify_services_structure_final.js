...
async function verifyServicesStructure() {
    ...
    try {
        ...
        // Vérifier les contraintes de vérification spécifiques
        ...
        // Tester l'insertion d'un service de sous-direction
        console.log('\n🧪 Test d\'insertion d\'un service de sous-direction...');
        const testSousDirectionQuery = `
            INSERT INTO services (id_ministere, libelle, description, type_service, direction_id, sous_direction_id, is_active)
            VALUES (1, 'Service Sous-direction Test', 'Service de test pour sous-direction', 'sous_direction', 1, 1, true)
            RETURNING id, libelle, type_service, direction_id, sous_direction_id;
        `;

        try {
            const testSousDirectionResult = await pool.query(testSousDirectionQuery);
            console.log('   ✅ Service de sous-direction créé:', testSousDirectionResult.rows[0]);

            // Supprimer le service de test
            await pool.query('DELETE FROM services WHERE libelle = $1', ['Service Sous-direction Test']);
            console.log('   ✅ Service de sous-direction supprimé');
        } catch (error) {
            if (error.code === '23503') {
                console.log('   ⚠️ Service de sous-direction non créé (sous-direction avec ID 1 n\'existe pas)');
            } else {
                throw error;
            }
        }
